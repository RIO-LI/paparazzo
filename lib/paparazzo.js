...
var Paparazzo = (function () {
    ...
    Paparazzo.prototype.addEventHandlers = function (eventName, handler, once, prepend) {
        ...
        this.splitEventNames(eventName)
            ...
            handlers = handlers.filter(function (fn) { return isFunction(fn); });
            if (once) {
                handlers = handlers.map(function (fn) {
                    var onceFn = function () {
                        var args = [];
                    ...
                    _this.handlerGuidsMap["" + ++guid] = onceFn;
                    fn['_guid'] = "" + guid;
                    return onceFn;
                });
            }
        ...
        if (!handler) {
            ...
            this.splitEventNames(eventName).forEach(function (name) {
                targetHandlers_1.forEach(function (targetHandler) {
                    var guid = targetHandler['_guid'];
                    var originFn = _this.handlerGuidsMap[guid];
                    var index = eventsMap.indexOf(originFn ? originFn : targetHandler);
                    if (index > -1) {
                        _this.eventsMap[name] = eventsMap = eventsMap.slice(0, index).concat(eventsMap.slice(index + 1));
                        delete _this.handlerGuidsMap[guid];
                    }
                });
            });
        }
